...
public class AutocompleteLocationsStage implements Stage {

  private Map<String, String> scriptParam;
  private Stage stage;
  private GDSClient api;
  private List<String> requiredFields = Arrays.asList("query", "limit");

  public AutocompleteLocationsStage(Stage stage, GDSClient api, Map<String, String> scriptParam) {
    this.stage = stage;
    this.api = api;
  ...
  public void workOut(Journal journal) {
    log.info("START AutocompleteLocationsStage >_");
    ...
    if (!listOfInvalidFields.isEmpty()) {
      ...
      LocationsAutocompleteResponse response =
          api.locationsAutocomplete(scriptParam.get("query"), Integer.valueOf(scriptParam.get("limit")), scriptParam.get("lang"), scriptParam.get("acceptLanguage"));
